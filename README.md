...
1. ```pagingAndIndicator``` 表示即使只有一张图片也可以轮播，并且如果设置了指示器的话也依然显示指示器。改值也是默认值。
2. ```canPagingNoIndicator``` 表示即使只有一张图片也可以轮播，但是无论你是否设置了指示器都不会显示。
3. ```canNotPagingHaveIndicator``` 表示如果只有一张图片则不能轮播也不会自动轮播，但是依然会显示指示器。如果你使用的是圆点型指示器则会显示一个被选中的圆点，如果是数字指示器则会显示当前页为1总页数也是1。
